...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier checkIsSeller(uint _sku){
    _;
    require(items[_sku].seller!= address(0) && items[_sku].seller == msg.sender);
  }

  modifier checkIsBuyer(uint _sku){
    _;
    require(items[_sku].buyer!= address(0) && items[_sku].buyer == msg.sender);
  }
